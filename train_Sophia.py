"""
train_Sophia.py - 使用Sophia优化器训练NanoGPT
"""
...
from model_Sophia import GPTConfig, GPTWithSophia
...
wandb_run_name = 'Sophia-G-v0-lr6-15B-BS480'
# data
dataset = 'openwebtext'
...
bias = False
# Sophia优化器配置
learning_rate = 4e-4  # 根据论文，Sophia的学习率通常比AdamW小一些
weight_decay = 1e-1
beta1 = 0.96  # Sophia论文中推荐的beta1
beta2 = 0.99  # Sophia论文中推荐的beta2
grad_clip = 1.0
rho = 1.0  # 裁剪阈值
update_freq = 10  # Hessian更新频率
estimator_type = 'gnb'  # 使用GNB估计器
subset_size = 240  # Hessian估计的子批次大小
# 学习率调度
decay_lr = True
warmup_iters = 2000  # 对于较小的模型，预热可以更短
# 训练目标
target_tokens = 5e7  # 针对较小模型设置适当的目标
# 系统设置
device = 'cuda'
...
if init_from == 'scratch':
    ...
    model = GPTWithSophia.from_pretrained(init_from, override_args)
    for k in ['n_layer', 'n_head', 'n_embd', 'block_size', 'bias', 'vocab_size']:
...
if wandb_log and master_process:
    ...
    # 添加Sophia特定指标
    wandb.define_metric("sophia/hessian_update_count", step_metric="iter")
    wandb.define_metric("sophia/clipped_ratio", step_metric="iter")
...
# Sophia特定状态
hessian_update_count = 0
clipped_params_ratio_history = []
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if wandb_log:
            ...
            log_data = {
                ...
                "val/loss": losses['val'],
                "train/loss_by_step": losses['train'],
                "val/loss_by_step": losses['val'],
                "lr": lr,
                "lr_step": lr,
                "mfu": running_mfu*100,
                ...
                "sophia/hessian_update_count": hessian_update_count,
            }
            
            # 如果有裁剪比率历史，添加最近的平均值
            if clipped_params_ratio_history:
                avg_clipped_ratio = sum(clipped_params_ratio_history[-100:]) / min(len(clipped_params_ratio_history), 100)
                log_data["sophia/clipped_ratio"] = avg_clipped_ratio
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        # 对于Sophia，我们需要手动执行优化步骤
        with ctx:
            logits, loss = model(X, Y)
            loss = loss / gradient_accumulation_steps
        
        # 执行Sophia优化步骤
        optimizer.step(model, None, X, Y)
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        # 计算Hessian更新频率
        if iter_num % update_freq == 1:
            hessian_update_count += 1
        
        print(f"iter {iter_num}: loss {lossf:.4f}, time {dt*1000:.2f}ms, mfu {running_mfu*100:.2f}%, "
              f"Hessian updates: {hessian_update_count}")
...
if master_process:
    ...
    print(f"Max memory allocated: {torch.cuda.max_memory_allocated() / (1024**3):.2f} GB")
    print(f"Total Hessian updates: {hessian_update_count}")